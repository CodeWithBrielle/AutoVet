...
export default function AiInsightPanels() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        const [apptResult, patientResult, invResult] = await Promise.all([
          fetch('/api/dashboard/appointment-forecast', { headers }).then(r => r.json()),
          fetch('/api/dashboard/patient-visit-predictions', { headers }).then(r => r.json()),
          fetch('/api/dashboard/inventory-forecast', { headers }).then(r => r.json())
        ]);

        setApptData(apptResult);
        setPatientData(patientResult);
        setInventoryData(invResult);
        setLoading(false);
      } catch (error) {
        console.error('Error fetching dashboard AI panels:', error);
        setLoading(false);
      }
    };
