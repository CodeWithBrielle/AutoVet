...
const eventStyles = {
  blue: "border-emerald-200 bg-emerald-100/70 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-300",
  ...
  return (
    <section className="card-shell h-[calc(100vh-11rem)] overflow-hidden p-0">
      <div className="flex h-full flex-col">
        <header className="flex flex-wrap items-center justify-between gap-3 border-b border-zinc-200 dark:border-dark-border px-5 py-4">
          <div className="flex flex-col gap-1">
            <h2 className="text-3xl font-bold tracking-tight text-zinc-900 dark:text-zinc-50">Clinic Calendar</h2>
          ...
          <div className="inline-flex items-center gap-2">
            ...
            <p className="min-w-[160px] text-center text-lg font-semibold text-zinc-800 dark:text-zinc-200">
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <div className="flex flex-1 flex-col overflow-hidden">
            <div className="grid grid-cols-7 border-b border-zinc-200 dark:border-dark-border bg-zinc-50 dark:bg-dark-surface">
              {weekDays.map((day) => (
                <div key={day} className="border-r border-zinc-200 dark:border-dark-border px-2 py-3 text-center text-sm font-semibold text-zinc-500 dark:text-zinc-400 last:border-r-0">
                  {day}
                </div>
              ))}
            </div>
              ...
              {calendarDays.map((entry, index) => {
                ...
                return (
                  <div
                    ...
                    className={clsx(
                      "cursor-pointer flex flex-col border-b border-r border-zinc-200 dark:border-dark-border p-2 last:border-r-0 transition-colors",
                      isSelected
                        ? "bg-emerald-50 dark:bg-emerald-900/20 ring-2 ring-inset ring-emerald-400"
                        : "hover:bg-zinc-50 dark:hover:bg-dark-surface/50 dark:bg-dark-surface"
                    )}
                  >
                    <p
                      className={clsx(
                        "inline-flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-full text-sm font-medium",
                        isTodayDate && "bg-emerald-600 font-bold text-white",
                        !isTodayDate && entry.inMonth && "text-zinc-700 dark:text-zinc-300",
                        !isTodayDate && !entry.inMonth && "text-zinc-400 dark:text-zinc-500"
                      )}
                    >
                      {entry.day}
                    </p>
                      ...
                      {entry.events.map((event) => {
                        ...
                        return (
                          <article
                            key={event.id}
                            className={clsx(
                              "rounded-lg border px-2 py-1 text-xs font-medium transition-all hover:scale-[1.02]",
          ...
          {/* Day Detail Panel */}
          {selectedDay && (
            <aside className="flex w-72 flex-col border-l border-zinc-200 dark:border-dark-border bg-white dark:bg-dark-card shadow-lg xl:w-80">
              {/* Panel header */}
              <div className="flex items-start justify-between border-b border-zinc-200 dark:border-dark-border px-4 py-4">
                <div>
                ...
                <button
                  ...
                  className="mt-1 rounded-lg p-1.5 text-zinc-400 dark:text-zinc-500 hover:bg-zinc-100 dark:hover:bg-dark-surface hover:text-zinc-600 dark:hover:text-zinc-300"
                >
              ...
              <div className="flex-1 overflow-y-auto p-4">
                {selectedDay.events.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-12 text-center">
                    <FiCalendar className="mb-3 h-10 w-10 text-zinc-300 dark:text-zinc-600" />
                    <p className="font-semibold text-zinc-500 dark:text-zinc-400">No appointments</p>
                    <p className="mt-1 text-sm text-zinc-400 dark:text-zinc-500">Nothing is booked for this day yet.</p>
                  </div>
                ) : (
