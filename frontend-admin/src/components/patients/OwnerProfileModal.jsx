...
export default function OwnerProfileModal({ ownerId, onClose, isOpen }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-zinc-900/60 p-4 backdrop-blur-sm dark:bg-zinc-950/70">
      <div className="w-full max-w-2xl overflow-hidden rounded-2xl bg-white shadow-2xl dark:bg-dark-card dark:shadow-dark-soft">
        ...
        <div className="p-6">
          {loading ? (
            <div className="py-12 text-center text-zinc-500">Loading profile data...</div>
          ) : owner ? (
            <div className="space-y-6">
              <div className="rounded-xl border border-zinc-200 bg-zinc-50 p-5 dark:border-dark-border dark:bg-dark-surface">
                <p className="text-2xl font-bold text-zinc-900 dark:text-zinc-50">{owner.name}</p>
                <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm text-zinc-600 dark:text-zinc-300">
                  {owner.phone && <p className="flex items-center gap-2"><FiPhone className="h-4 w-4 text-zinc-400" />{owner.phone}</p>}
                  {owner.email && <p className="flex items-center gap-2"><FiMail className="h-4 w-4 text-zinc-400" />{owner.email}</p>}
                  {owner.address && <p className="flex items-start gap-2 sm:col-span-2"><FiMapPin className="h-4 w-4 mt-0.5 text-zinc-400" />{`${owner.address}, ${owner.city} ${owner.zip || ''}`}</p>}
                </div>
              </div>

              <div>
                <h4 className="mb-3 flex items-center gap-2 text-lg font-bold text-zinc-900 dark:text-zinc-50">
                  <LuPawPrint className="h-5 w-5 text-emerald-500" />
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {owner.pets.map((pet) => (
                      ...
                      >
                        ...
                        />
                        <div>
                          <p className="font-bold text-zinc-900 dark:text-zinc-50">{pet.name}</p>
                          <p className="text-xs text-zinc-500 dark:text-zinc-400">{pet.species?.name} • {pet.breed?.name}</p>
                        </div>
                      </Link>
                    ))}
                  </div>
