  ...
  if (isLoading && species.length === 0) {
    return (
      <div className="flex h-64 items-center justify-center">
        <div className="text-zinc-500 animate-pulse font-medium italic tracking-widest text-xs uppercase">Syncing clinical benchmarks...</div>
      </div>
    );
  }
  ...
  if (species.length === 0) {
    return (
      <div className="card-shell p-12 text-center border-dashed border-2">
        <FiAlertTriangle className="mx-auto h-12 w-12 text-zinc-300 mb-4" />
        <h3 className="text-lg font-bold text-zinc-800 dark:text-zinc-100">No Species Configured</h3>
          ...
          <div className="w-full max-w-md rounded-3xl bg-white p-8 dark:bg-dark-card border border-zinc-200 dark:border-dark-border shadow-2xl animate-in fade-in zoom-in duration-200">
            ...
            <form onSubmit={handleSave} className="space-y-5">
              <div className="grid grid-cols-1 gap-5">
                <div>
                  <label className="block text-sm font-bold text-zinc-700 dark:text-zinc-300 mb-2">Label / Tier</label>
                  <select 
                    required
