...
function SimpleLineChart({ data, width = 600, height = 240, color = "#10b981", dataKey = "total", prefix = "" }) {
  if (!data || !Array.isArray(data) || data.length === 0) {
    return <div className="h-[240px] flex items-center justify-center text-zinc-400 font-medium bg-zinc-50/50 dark:bg-zinc-900/50 rounded-xl">Insufficient historical data</div>;
  }
...
function SimpleBarChart({ data, width = 600, height = 300, color = "#6366f1", prefix = "" }) {
  if (!data || !Array.isArray(data) || data.length === 0) {
    return <div className="h-[300px] flex items-center justify-center text-zinc-400 font-medium bg-zinc-50/50 dark:bg-zinc-900/50 rounded-xl">No distribution data found</div>;
  }
...
function InterpretationPanel({ report }) {
  ...
  return (
    <div className="bg-slate-50 dark:bg-zinc-900/50 border border-slate-200 dark:border-zinc-800 rounded-xl p-6 mt-6">
      ...
      {hasInterpretation ? (
        <p className="text-sm font-medium text-zinc-700 dark:text-zinc-300 leading-relaxed mb-4">{report.interpretation}</p>
      ) : (
        <p className="text-sm font-medium text-zinc-400 dark:text-zinc-500 italic mb-4">
        ...
        <section className="animate-in fade-in slide-in-from-bottom-4 duration-500 w-full space-y-8">
          {inventoryData.summary?.data ? (
            ...
            <div className="h-96 flex flex-col items-center justify-center text-center space-y-4">
               <FiIcons.FiInfo className="w-12 h-12 text-zinc-300" />
               <p className="text-xl font-black text-zinc-400 uppercase tracking-widest">No Inventory Insights Available Yet</p>
               <p className="max-w-xs text-sm text-zinc-400 font-medium">Add stock to the clinical database to unlock deep inventory reports.</p>
               <button onClick={() => window.location.href = '/inventory'} className="mt-4 text-emerald-600 font-black uppercase tracking-widest text-xs hover:underline">Go to Inventory Module</button>
            </div>
          )}
        </section>
      )}
